 ...
 * @author Alecs
 */
public class TaquillaController extends ControladorGenerico<TaquillaDAO, TaquillaVO>{

    public TaquillaController() throws ZologicoException{
        try {
            this.setCnn(ConexionBD.obtenerConexionBD());
            this.setDao(new TaquillaDAO(this.getCnn()));
    ...
    public TaquillaController(Connection cnn) throws ZologicoException {
        try {
            this.setCnn(cnn);
            this.setDao(new TaquillaDAO(this.getCnn()));
        }catch (Exception ex) {
