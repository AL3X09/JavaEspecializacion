 ...
 * @author Alecs
 */
public class AnimalController extends ControladorGenerico<AnimalDAO, AnimalVO>{

    public AnimalController() throws ZologicoException{
        try {
            this.setCnn(ConexionBD.obtenerConexionBD());
            this.setDao(new AnimalDAO(this.getCnn()));
    ...
    public AnimalController(Connection cnn) throws ZologicoException {
        try {
            this.setCnn(cnn);
            this.setDao(new AnimalDAO(this.getCnn()));
        }catch (Exception ex) {
